...
export default function Breadcrumbs({
  ...
  const navigation = useMemo(() => ({ items: getMenuItemsForGroup(groupConfig) }), [groupConfig]);
  ...
  if (!custom && main && main.type === 'collapse' && main.breadcrumbs === true) {
    ...
    mainContent = (
      ...
      >
        ...
        {main.breadcrumbTitle ?? main.title}
      </Typography>
    );
    breadcrumbContent = (
      <MainCard
  ...
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || custom) {
    itemTitle = (item?.breadcrumbTitle ?? item?.title) as NavItemType['title'];
        ...
        >
          ...
          >
            {title && !titleBottom && (
              <Grid item>
                <Typography variant={isMobile ? 'h3' : 'h2'} sx={{ fontWeight: 700 }}>
                  {custom ? heading : (item?.breadcrumbTitle ?? item?.title)}
                </Typography>
              </Grid>
            )}
